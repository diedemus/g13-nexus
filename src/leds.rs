...
pub fn set_rgb(rgb: [u8; 3]) -> Result<()> {
    let b = Path::new(RGB);
    fs::write(
        b.join("multi_intensity"),
        format!("{} {} {}\n", rgb[0], rgb[1], rgb[2]),
    )
    .context("write G13 multi_intensity")?;
    let m = fs::read_to_string(b.join("max_brightness")).unwrap_or_else(|_| "255".into());
    fs::write(b.join("brightness"), m.trim()).context("write G13 brightness")?;
    Ok(())
}
fn led(name: &str, on: bool) -> Result<()> {
    fs::write(
...
pub fn set_bank(bank: u8) -> Result<()> {
    for i in 1..=3 {
        led(&format!("g13:red:macro_preset_{i}"), i == bank)?;
    }
    Ok(())
}
pub fn set_record(on: bool) -> Result<()> {
    led("g13:red:macro_record", on)
}
